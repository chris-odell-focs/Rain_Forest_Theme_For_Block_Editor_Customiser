...
/*
Plugin Name: Foxdell Folio BEC Theme Rain Forest
Plugin URI: 
Description: A theme for the Foxdell Folio Block Editor Customer inspired by the rain forests
Version: 1.0.0
Author: Foxdell Folio
Author URI: 
License: GPLv2 or later
Text Domain: fofobecrainforest
*/

function fofo_bec_theme_rain_forest_register_theme( $theme_register ) {

    $theme = $theme_register->get_new_theme();
    $theme->name = 'fofo_bec_rain_forest';
    $theme->display_name = 'FOFO Bec Rain Forest';
    $theme->settings_page = 'fofo_bec_theme_rain_forest_settings_page';
    $theme->css = esc_url( plugins_url( 'style.css', __FILE__ ) );
    $theme->my_location = dirname( __FILE__ ).'/fofo-bec-theme-rain-forest.php';

    $theme_register->register_theme( $theme );

}

function fofo_bec_theme_rain_forest_settings_page( $page_builder ) {
...
add_action('fofo_bec_register_theme',"fofo_bec_theme_rain_forest_register_theme");
 ...
 * @since 1.5.0
 */
function fofo_bec_theme_rain_forest_activate() {

    set_transient( 'fofo_bec_theme_rain_forest_activate', true, 5 );
}

register_activation_hook( __FILE__, 'fofo_bec_theme_rain_forest_activate' );
 ...
 * @since 0.1.0
 */
function fofo_bec_theme_rain_forest_admin_notice(){

    /* Check transient, if available display notice */
    if( get_transient( 'fofo_bec_theme_rain_forest_activate' ) ){
        ?>
...
/* Add admin notice */
add_action( 'admin_notices', 'fofo_bec_theme_rain_forest_admin_notice' );
